...
def build_registry(settings: AppSettings, bus: EventBus) -> ServiceRegistry:
    ...
    registry.register(
        MissionEngine,
        lambda r: MissionEngine(r.resolve(EventBus)),
        name="missions",
        critical=True,
    )
    registry.register(
    ...
    registry.register(
        ...
        lambda r: NetworkService(
            ...
            settings.backend,
            missions=r.resolve(MissionEngine),
        ),
        name="network",
        depends_on=[StateManager, IdentityService, CapabilityManager, MissionEngine],
        lazy=False,
    )
    return registry
...
def main(argv: list[str] | None = None) -> int:
    ...
    missions = registry.resolve(MissionEngine)
    network = registry.try_resolve(NetworkService)
    window = MainWindow(
        bus, state, clock, theme, settings,
        missions=missions,
        capabilities=registry.resolve(CapabilityManager),
        sender=network,
    )
    ...
    if settings.app.developer_tools:
        window.attach_developer_console(
            DeveloperConsole(bus, state, clock, registry, theme, network, missions)
        )
