...
from jarvis_protocol.errors import is_retryable
...
#: Tipi appartenenti al livello operativo, instradati al Mission Engine.
_OPERATIONS_TYPES: frozenset[str] = frozenset(
    {
        MissionType.MISSION_STARTED,
        MissionType.MISSION_UPDATED,
        MissionType.MISSION_FINISHED,
        MissionType.ACTION_QUEUED,
        MissionType.ACTION_STARTED,
...
class MessageDispatcher:
    ...
    def dispatch(self, envelope: Envelope) -> None:
        ...
        try:
            ...
            # Il livello operativo ha un proprietario unico: il motore. Il
            # dispatcher valida e inoltra, senza interpretare.
            if envelope.type in _OPERATIONS_TYPES:
                self._dispatch_operations(envelope)
                return
    ...
    def _dispatch_operations(self, envelope: Envelope) -> None:
        """Valida e inoltra un messaggio operativo al motore."""
        if self._missions is None:
            _log.debug("Messaggio operativo ignorato: motore non collegato")
            return
        payload = parse_payload(envelope.type, envelope.payload)
        if payload is None:
            _log.warning("Messaggio operativo '%s' non conforme: scartato", envelope.type)
            return
        self._missions.apply(envelope.type, payload)
    ...
    def _on_task_update(self, envelope: Envelope) -> None:
        """Forma semplice, retrocompatibile.

        ``task.update`` precede il livello operativo. Invece di mantenere due
        rappresentazioni parallele nella GUI — il modo piu' rapido per farle
        divergere — lo si proietta anche come azione senza missione.
        """
        ...
        self._project_task_as_action(payload, event)

    def _project_task_as_action(self, payload: TaskUpdatePayload, event: EventType) -> None:
        """Rende visibile un ``task.update`` anche nella coda delle azioni."""
        if self._missions is None:
            return
        if event is EventType.TASK_STARTED:
            self._missions.apply(
                MissionType.ACTION_QUEUED,
                {"action_id": payload.task_id, "tool": "task", "title": payload.label},
            )
            self._missions.apply(MissionType.ACTION_STARTED, {"action_id": payload.task_id})
        elif event is EventType.TASK_FINISHED:
            self._missions.apply(
                MissionType.ACTION_FINISHED,
                {
            ...
            self._missions.apply(
                MissionType.ACTION_PROGRESS,
                {"action_id": payload.task_id, "progress": payload.progress},
            )
